$(() => {
    ...
    $('#trip-imgs').on('slid.bs.carousel', function (event) {
        const total = $('#trip-imgs .carousel-item').length;
        const imgs = $('#trip-imgs .carousel-item img');
        const prev = event.from;
        const next = (event.from + 2) % total;
        $('#prev-item').css('background-image', `url(${imgs[prev].src})`);
        $('#next-item').css('background-image', `url(${imgs[next].src})`);
    })
